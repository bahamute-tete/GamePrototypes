...
// rd Ray Direction
// rl Ray Length
struct sRay   { vec3 ro ; vec3  rd ; float sd; float rl; };//rayOrigin, rayDirection, startDistance, rayLength
struct sHit   { vec3 hp ; float hd ; vec3 oid; };//hitPoint ,hitDistance,objectID
struct sSurf  { vec3 nor; vec3  ref; vec3 tra; };//normal , reflection, transmission
struct sMat   { vec3 ctc; float frs; float smt; vec2 par; float trs; float fri; };
struct sShade { vec3 dfs; vec3  spc; };//diffuse specular
struct sLight { vec3 rd ; vec3  col; };//raydirction color
